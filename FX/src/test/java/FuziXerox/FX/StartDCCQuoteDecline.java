...
import log.Log;
import pagelibrary.DCCQuote;
import report.HtmlReport;
import testBase.TestBase;

public class StartDCCQuoteDecline extends TestBase {
	
	private DCCQuote dCCQuote;
	
	
	@DataProvider(name = "loginData")
	public String[][] getTestData() {
		GlobalVariables.setStrStartTime(new SimpleDateFormat("yyyy-MM-dd-HH-mm-ss").format(new Date()));
		GlobalVariables.setTestCaseName("StartDCCQuoteDecline");
	...
	{
		init();
		dCCQuote = new DCCQuote (driver);
		dCCQuote.jobCreation();
		dCCQuote.assignToSite();
		dCCQuote.quoteApproval();
		dCCQuote.quoteDecline();
	}
